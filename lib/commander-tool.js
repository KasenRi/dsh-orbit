...
export function createCommanderDecisionTool(capture) {
    return defineTool({
        name: ORBIT_COMMANDER_DECISION_TOOL,
        description: 'Orbit 指挥官内部决策提交工具。仅用于提交当前 PLAN / STEP_EVALUATE / FINAL_EVALUATE / STRATEGY_RECONSIDER 的结构化结果；调用后当前轮次结束。',
        parameters: {
            mode: {
                type: 'string',
                required: true,
        ...
        output: {
            ...
            render: () => [{ type: 'text', text: 'Orbit Commander decision accepted.' }],
        },
