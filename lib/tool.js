    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (args.action === 'status')
                return (await service.status(cwd, ownerSessionId));
            if (args.action === 'stop')
                return (await service.stop(args.run_id, cwd, ownerSessionId));
            if (args.action === 'doctor')
                return (await service.doctor(cwd));
            if (args.action === 'resume')
                return (await service.resume(input, cwd, exec.signal, ownerSessionId));
            return (await service.run(input, cwd, exec.signal, ownerSessionId));
        },
    });
}
