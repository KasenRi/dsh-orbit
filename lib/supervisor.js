...
import { assertCommanderDecision, assertStrategyDecision, assertTimeoutDecision, assertWatchdogDecision, assertGuardWatchdogDecision, assertHeartbeatDecision, assertFinalAuditDecision, COMMANDER_FINAL_EVALUATE_SCHEMA, COMMANDER_PLAN_SCHEMA, COMMANDER_STEP_EVALUATE_SCHEMA, COMMANDER_STRATEGY_SCHEMA, WATCHDOG_GUARD_SCHEMA, WATCHDOG_HEARTBEAT_SCHEMA, WATCHDOG_FINAL_AUDIT_SCHEMA, WATCHDOG_RUNTIME_SCHEMA, WATCHDOG_STRATEGY_SCHEMA, WATCHDOG_TIMEOUT_SCHEMA, } from "./decisions.js";
import { ORBIT_COMMANDER_DECISION_TOOL, ORBIT_RUN_COMPLETE_TOOL } from "./host.js";
import { OrbitMoaAdapter } from "./moa-adapter.js";
import { applyCommanderNeedsUser, applyCorrectionStep, applyExecutorCapabilityUnavailable, applyExecutorInterrupted, applyExecutorResume, applyExecutorSuccess, applyFinalAppend, applyFinalAuditApproved, applyFinalAuditBlocked, applyFinalCandidate, applyFinalSuccess, applyTerminalConfirmation, completionGateIssue, applyPlan, applyStepPass, baseStepIdOf, beginStep, clearExecutorChild, clearGuardRecovery, correctionBlockCode, correctionDepthOf, createInitialState, enterBudgetExhausted, enterNeedsUser, ensureAutomaticLoopBudgetForPlan, hashGoal, markStrategyChallengeUsed, markMeaningfulProgress, normalizePlan, assertMoaPlanWithinPolicy, normalizeExecutionMode, openWatchdogAttempt, recordGuardRecovery, recordPlanFailure, recordWatchdogDecision, restoreEvaluationState, resumeFromNeedsUser, stopRun, upsertStepResult, normalizeCapabilities, } from "./kernel.js";
...
import { COMMANDER_EXTENSION_MS, COMMANDER_HARD_CEILING_MS, COMMANDER_SOFT_DEADLINE_MS, DEFAULT_CAPABILITIES, DEFAULT_HEARTBEAT_HEALTHY_INTERVAL_MS, DEFAULT_HEARTBEAT_INTERVAL_MS, DEFAULT_HEARTBEAT_SUSPECT_INTERVAL_MS, EXECUTOR_TIMEOUT_MS, GUARD_ESCALATION_THRESHOLD, GUARD_FIRST_INSTRUCTION, GUARD_NEEDS_USER_INSTRUCTION, GUARD_RECOVERY_CAP, GUARD_REPEAT_INSTRUCTION, GUARD_RETRY_INSTRUCTION, MAX_EXECUTOR_INTERRUPT_RETRIES, MAX_FINAL_AUDIT_BLOCKS, MAX_WATCHDOG_CALLS_PER_STEP, WATCHDOG_TIMEOUT_MS, } from "./types.js";
...
模式：${mode}
遥测：${JSON.stringify(telemetry ?? {})}`;
const WATCHDOG_HEARTBEAT_PROMPT = (snapshot) => `你是 Orbit 心跳监控模型（Heartbeat Watchdog），当前阶段：HEARTBEAT_REVIEW。
...
const WATCHDOG_FINAL_AUDIT_PROMPT = (goal, state, fingerprint) => `你是 Orbit 最终关闭审计 Watchdog，当前阶段：FINAL_AUDIT。
...
const COMMANDER_TERMINAL_CONFIRM_PROMPT = (goal, auditReason) => `你是 Orbit 指挥官（Commander），当前阶段：TERMINAL_CONFIRM。
...
export class OrbitSupervisor {
    ...
    grantFingerprint(filter) {
        return [...(filter.allow ?? [])].sort().join('|');
    }
    resumeRoleId(state, role, fingerprint) {
        const session = state.role_sessions?.[role];
        if (!session || session.needs_rotation === true)
            return undefined;
        if (role === 'executor' && fingerprint !== undefined && session.grant_fingerprint !== fingerprint)
            return undefined;
        return session.child_id || undefined;
    }
    bindRoleSession(state, role, childId, options = {}) {
        state.role_sessions ??= {};
        ...
        if (options.fingerprint !== undefined)
            next.grant_fingerprint = options.fingerprint;
        if (options.stepId !== undefined)
            next.last_step_id = options.stepId;
        state.role_sessions[role] = next;
    }
    rotateRoleSession(state, role, reason) {
    ...
    recordRoleTurn(state, role, usage, stepId) {
        const current = state.role_sessions?.[role];
        if (!current)
            return;
        current.turns += 1;
        if (stepId !== undefined)
            current.last_step_id = stepId;
        if (!usage)
            return;
    ...
    ensureHeartbeatState(state) {
        if (!state.progress)
            state.progress = { seq: 0, at: new Date(this.now()).toISOString() };
    ...
    heartbeatTargetFingerprint(state, target, childId) {
        return hashGoal(JSON.stringify({
            run_id: state.run_id,
            phase: state.phase,
    ...
    invariantIssues(state) {
        const issues = [];
        const running = state.plan.steps.filter((step) => step.status === 'running');
        if (running.length > 1)
            issues.push('ORBIT_INVARIANT_MULTIPLE_RUNNING_STEPS');
        ...
        if (state.phase === 'FINAL_VERIFY' || state.phase === 'TERMINAL_CONFIRM') {
            if (state.plan.steps.some((step) => step.status !== 'passed'))
                issues.push('ORBIT_INVARIANT_FINAL_WITH_UNPASSED_STEP');
        }
    ...
    heartbeatSnapshot(state, target, telemetry, childId) {
    ...
    scheduleNextHeartbeat(state, decision) {
        const heartbeat = this.ensureHeartbeatState(state);
    ...
    async heartbeatReview(state, target, telemetry, childId, signal) {
        const heartbeat = this.ensureHeartbeatState(state);
        ...
        let decision;
        try {
            decision = assertHeartbeatDecision(result.structured);
        }
        catch (error) {
            heartbeat.last_decision = 'UNAVAILABLE';
        ...
        if (decision.decision === 'STRATEGY_REVIEW')
            heartbeat.strategy_review_requested = true;
        this.scheduleNextHeartbeat(state, decision.decision);
        this.store.writeState(state);
        return decision;
    }
    async waitWithHeartbeat(state, target, work, timeoutMs, signal, handle) {
        const heartbeat = this.ensureHeartbeatState(state);
        ...
        for (;;) {
            ...
            if (!decision || decision.decision === 'HEALTHY' || decision.decision === 'WAIT' || decision.decision === 'STRATEGY_REVIEW')
                continue;
            const reason = decision.reason ?? decision.decision;
            if (decision.decision === 'RESTART_STEP') {
                if (target === 'commander')
                    return { kind: 'rotate_commander', reason: `Watchdog requested RESTART_STEP while Commander was active: ${reason}` };
                return { kind: 'restart_step', reason };
            }
            if (decision.decision === 'ROTATE_COMMANDER') {
                if (target === 'commander')
                    return { kind: 'rotate_commander', reason };
     ...
     */
    async runCommander(state, mode, prompt, outputSchema, signal, heartbeatRotations = 0) {
        const startedAt = this.now();
        const commanderTools = [
            ...this.config.commanderReadOnlyTools.filter((name) => READ_ONLY_ROLE_TOOLS.includes(name)),
        ...
        const start = (childId) => this.host.startRole({
            role: 'commander',
    ...
    async prepareMoaStep(state, step, signal) {
        ...
        try {
            if (state.moa_step.phase === 'FANOUT') {
                ...
                const waited = await this.waitWithHeartbeat(state, 'moa', pending, this.config.executorTimeoutMs ?? EXECUTOR_TIMEOUT_MS, phaseSignal);
                if (waited.kind !== 'work') {
                    controller.abort();
                    ...
                    if (waited.kind === 'aborted' || signal?.aborted)
                        return { ready: false, done: true, message: 'ORBIT_ABORTED' };
            ...
            if (state.moa_step.phase === 'PROMOTING') {
                ...
                const receipt = await this.host.withWorkspaceMutationLease(workspace, signal, () => this.moa.promote({ workspace, runId: state.run_id, stepId: step.id, winningCandidate: winner }));
    ...
    async awaitExecutor(state, handle, signal) {
    ...
    async runFinalAudit(state, signal) {
        const heartbeat = this.ensureHeartbeatState(state);
        ...
        let decision;
        try {
            decision = assertFinalAuditDecision(result.structured);
        }
        catch (error) {
        ...
        if (decision.decision === 'APPROVE_CLOSE') {
            ...
            if (state.commander?.remaining_gap)
                state.commander.remaining_gap = undefined;
            if (state.last_error)
    ...
    async runTerminalConfirm(state, signal, heartbeatRotations = 0) {
        ...
        try {
            try {
                ...
                if (!resumeOf)
                    throw error;
                this.rotateRoleSession(state, 'commander', 'TERMINAL_CONFIRM_RESUME_FAILED:' + truncateSafe(error instanceof Error ? error.message : String(error), 120));
                this.store.writeState(state);
                handle = await start();
            }
            if (handle.childId) {
            ...
            const raced = await this.waitWithHeartbeat(state, 'commander', handle.result, COMMANDER_SOFT_DEADLINE_MS, signal, handle);
            if (raced.kind === 'rotate_commander') {
                await this.cancelHandle(handle, 'HEARTBEAT_ROTATE_COMMANDER');
                this.rotateRoleSession(state, 'commander', 'HEARTBEAT_ROTATE_COMMANDER:' + truncateSafe(raced.reason, 120));
                this.store.writeState(state);
                if (heartbeatRotations < 1)
                    return this.runTerminalConfirm(state, signal, heartbeatRotations + 1);
            ...
            const submission = raced.value.structured;
    ...
    async applyCommanderOutcome(state, outcome, step, final, signal) {
        ...
        if (state.heartbeat_watchdog?.strategy_review_requested)
            state.heartbeat_watchdog.strategy_review_requested = false;
        if (decision.executor_session === 'RESET')
            this.rotateRoleSession(state, 'executor', 'COMMANDER_REQUEST');
        if (decision.decision === 'NEEDS_USER') {
        ...
        if (decision.decision === 'SUCCESS') {
            if (this.config.finalAuditEnabled !== true) {
                ...
                if (outcome.output.trim())
                    state.commander = { ...state.commander, final_output: outcome.output.trim().slice(0, 8000) };
                this.markProgress(state);
                this.store.writeState(state);
