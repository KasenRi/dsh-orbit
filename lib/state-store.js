...
export class OrbitStateStore {
    ...
    lockDepth = 0;
    onWrite;
    constructor(projectDir, onWrite, sessionId) {
    ...
    static hasAnyActiveRun(projectDir) {
    ...
    legacyOwnedBySession() {
        if (this.sessionId === undefined || !existsSync(this.legacyStatePath))
            return false;
    ...
    sourcePathForRead() {
        ...
        if (this.legacyOwnedBySession())
            return this.legacyStatePath;
        return undefined;
    }
    /**
     ...
     */
    adoptOwnerlessLegacyState() {
        if (this.sessionId === undefined || existsSync(this.statePath) || !existsSync(this.legacyStatePath))
            return null;
        return this.transact(() => {
            if (existsSync(this.statePath) || !existsSync(this.legacyStatePath))
                return null;
            let parsed;
            try {
                parsed = JSON.parse(readFileSync(this.legacyStatePath, 'utf8'));
            }
            catch (error) {
            ...
            if (raw['owner_session_id'] !== undefined)
                return null;
            const migrated = {
                ...raw,
