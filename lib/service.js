...
import { createCommanderDecisionTool } from "./commander-tool.js";
import { createRunCompleteTool } from "./completion-tool.js";
...
export class OrbitService extends Service {
    ...
    stateStore(projectDir, ownerSessionId = this.currentOwnerSessionId(), publish = false) {
        return new OrbitStateStore(projectDir, publish ? (state) => this.publishRuntime(state) : undefined, ownerSessionId);
    }
    executionKey(projectDir, ownerSessionId) {
        return `${projectDir}\u0000${ownerSessionId ?? '<ownerless>'}`;
    }
    supervisorConfig(ownerSessionId, ownerlessLegacy = false) {
        return {
    ...
    ownerlessLegacyState(projectDir) {
    ...
    run(input, projectDir, signal, explicitOwnerSessionId) {
    ...
    async execute(dir, ownerSessionId, signal, operation) {
        const key = this.executionKey(dir, ownerSessionId);
        if (this.executions.has(key))
            return { ok: false, action: 'run', message: 'ORBIT_MUTATION_DRIVER_CONFLICT: 当前 Session 已有 Orbit 执行中的调用。' };
        ...
        try {
            ...
            if (settled.phase === 'SUCCESS' || settled.phase === 'STOPPED' || settled.phase === 'BUDGET_EXHAUSTED' || settled.message === 'ORBIT_ABORTED') {
                if (ownerSessionId !== undefined)
                    await this.host.revokeOwner(dir, ownerSessionId);
                else
    ...
    async resume(input, projectDir, signal, explicitOwnerSessionId) {
        const dir = this.resolveProjectDir(projectDir);
        const ownerSessionId = explicitOwnerSessionId ?? this.currentOwnerSessionId();
        const store = this.stateStore(dir, ownerSessionId);
        let state = store.readState();
        if (!state && ownerSessionId !== undefined)
            state = store.adoptOwnerlessLegacyState();
        if (!state)
            return { ok: false, action: 'resume', message: 'ORBIT_RUN_NOT_FOUND: 当前 Session 没有可继续的持久化 Run。' };
        ...
        if (state.phase === 'NEEDS_USER') {
            if (state.owner_session_id !== undefined)
                return this.run(input, dir, signal, ownerSessionId);
    ...
    async stop(runId, projectDir, explicitOwnerSessionId) {
        const dir = this.resolveProjectDir(projectDir);
        const ownerSessionId = explicitOwnerSessionId ?? this.currentOwnerSessionId();
        ...
        if (runId && state?.run_id !== runId)
            return { ok: false, action: 'stop', message: 'ORBIT_RUN_NOT_FOUND: 当前 Session 的 Run id 不匹配。' };
    ...
    status(projectDir, explicitOwnerSessionId) {
        const dir = this.resolveProjectDir(projectDir);
        const ownerSessionId = explicitOwnerSessionId ?? this.currentOwnerSessionId();
    ...
    recordGuardBlock(code, reason, projectDir, ownerSessionId) {
    ...
    githubAllowed(projectDir, ownerSessionId) {
